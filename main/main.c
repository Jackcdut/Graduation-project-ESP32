...
void app_main(void)
{
    ...
    /* Initialize STM32 communication module */
    ESP_LOGI(TAG, "Initializing STM32 communication...");
    esp_err_t stm32_ret = stm32_comm_init(NULL);
    if (stm32_ret == ESP_OK) {
        ESP_LOGI(TAG, "STM32 communication initialized successfully");
    } else {
        ESP_LOGW(TAG, "STM32 communication init failed: %s", esp_err_to_name(stm32_ret));
    }
