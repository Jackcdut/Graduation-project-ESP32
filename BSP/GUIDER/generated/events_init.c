...
/* Get amplitude slider reference */
lv_obj_t* scrCopy_get_amplitude_slider(void)
{
	return scrCopy_slider_amp;
}
...
static void scrCopy_event_handler (lv_event_t *e)
{
	...
	case LV_EVENT_SCREEN_LOADED:
	{
		...
		/* 切换STM32到信号发生器模式 */
		stm32_set_mode(STM32_MODE_SIGGEN);
		ESP_LOGI("SIGGEN_UI", "Signal generator screen loaded, mode switched to SIGGEN");
		break;
	}
	case LV_EVENT_SCREEN_UNLOADED:
	{
		/* 页面卸载时切换回空闲模式 */
		stm32_set_mode(STM32_MODE_IDLE);
		ESP_LOGI("SIGGEN_UI", "Signal generator screen unloaded, mode switched to IDLE");
...
static void scrCopy_sliderBright_event_handler (lv_event_t *e)
{
	...
	case LV_EVENT_VALUE_CHANGED:
	{
		...
		/* Send command to STM32 signal generator */
		float amp_volts = amplitude / 100.0f;  /* Convert to Vpp */
		stm32_siggen_set_all((uint8_t)current_wave_type, (float)frequency, amp_volts);
		break;
	}
	default:
		break;
...
static void scrCopy_sliderHue_event_handler (lv_event_t *e)
{
	...
	case LV_EVENT_CLICKED:
	{
		...
		/* Send command to STM32 signal generator */
		float amp_volts = amplitude / 100.0f;
		stm32_siggen_set_all(WAVE_SQUARE, (float)frequency, amp_volts);
		break;
	}
	default:
		break;
...
static void scrCopy_btnNext_event_handler (lv_event_t *e)
{
	...
	case LV_EVENT_CLICKED:
	{
		...
		/* Send command to STM32 signal generator */
		float amp_volts = amplitude / 100.0f;
		stm32_siggen_set_all(WAVE_SINE, (float)frequency, amp_volts);
		break;
	}
	default:
		break;
...
static void scrCopy_imgColor_event_handler (lv_event_t *e)
{
	...
	case LV_EVENT_CLICKED:
	{
		...
		/* Send command to STM32 signal generator */
		float amp_volts = amplitude / 100.0f;
		stm32_siggen_set_all(WAVE_TRIANGLE, (float)frequency, amp_volts);
		break;
	}
	default:
		break;
...
/* Apply button event handler - sends signal generator settings to STM32 */
static void scrCopy_btnApply_event_handler (lv_event_t *e)
{
	lv_event_code_t code = lv_event_get_code(e);
	...
	case LV_EVENT_CLICKED:
	{
		/* Get current wave type from button states */
		...
		int32_t freq_slider_val = lv_slider_get_value(guider_ui.scrCopy_sliderBright);
		uint32_t frequency;
		if (freq_slider_val <= 100) {
			frequency = freq_slider_val * 10;
		} else if (freq_slider_val <= 190) {
			frequency = 1000 + (freq_slider_val - 100) * 100;
		} else if (freq_slider_val <= 590) {
			frequency = 10000 + (freq_slider_val - 190) * 250;
		} else {
			frequency = 110000 + (freq_slider_val - 590) * 4610;
		}

		/* Get amplitude - need to find the amplitude slider */
		extern lv_obj_t* scrCopy_get_amplitude_slider(void);
		lv_obj_t *amp_slider = scrCopy_get_amplitude_slider();
		float amplitude = 1.5f;  /* Default */
		if (amp_slider && lv_obj_is_valid(amp_slider)) {
			uint16_t amp_val = lv_slider_get_value(amp_slider);
			amplitude = amp_val / 100.0f;  /* Convert to Vpp */
		}

		/* Send command to STM32 */
		esp_err_t ret = stm32_siggen_set_all((uint8_t)wave_type, (float)frequency, amplitude);
		
		/* Show toast notification */
		const char *wave_names[] = {"Sine", "Triangle", "Square"};
		char toast_msg[64];
		if (ret == ESP_OK) {
			snprintf(toast_msg, sizeof(toast_msg), LV_SYMBOL_OK " %s %.0fHz %.2fV", 
			         wave_names[wave_type], (float)frequency, amplitude);
			show_toast(toast_msg, true, 1500);
		} else {
			show_toast(LV_SYMBOL_CLOSE " Send Failed", false, 2000);
		}
		break;
	}
	default:
...
void events_init_scrCopy(lv_ui *ui)
{
	...
	lv_obj_add_event_cb(ui->scrCopy_btnApply, scrCopy_btnApply_event_handler, LV_EVENT_ALL, ui);
}
...
static void scrPrintMenu_event_handler (lv_event_t *e)
{
	...
	case LV_EVENT_SCREEN_LOADED:
	{
		...
		/* 切换STM32到万用表模式，开始上报测量数据 */
		stm32_set_mode(STM32_MODE_MULTIMETER);
		ESP_LOGI("DMM_UI", "Multimeter screen loaded, mode switched to MULTIMETER");
		break;
	}
	case LV_EVENT_SCREEN_UNLOADED:
	{
		/* 页面卸载时切换回空闲模式 */
		stm32_set_mode(STM32_MODE_IDLE);
		ESP_LOGI("DMM_UI", "Multimeter screen unloaded, mode switched to IDLE");
		break;
	}
	default:
		break;
...
static float ps_current_set = 1.0f;       // Current limit setting (0-2A)
...
static void scrPowerSupply_event_handler (lv_event_t *e)
{
	...
	case LV_EVENT_SCREEN_LOADED:
	{
		...
		/* 初始化电源模块并注册数据回调 */
		extern esp_err_t ps_global_init(void);
		ps_global_init();
		
		/* 切换STM32到电源模式，开始上报电源数据 */
		stm32_set_mode(STM32_MODE_POWER_SUPPLY);
		
		ESP_LOGI("POWER_UI", "Power supply screen loaded, mode switched to POWER");
		break;
	}
	case LV_EVENT_SCREEN_UNLOADED:
	{
		/* 页面卸载时切换回空闲模式 */
		stm32_set_mode(STM32_MODE_IDLE);
		ESP_LOGI("POWER_UI", "Power supply screen unloaded, mode switched to IDLE");
		break;
	}
	default:
		break;
...
static void scrPowerSupply_switchPower_event_handler (lv_event_t *e)
{
	...
	case LV_EVENT_VALUE_CHANGED:
	{
		...
		/* 发送命令到STM32 */
		stm32_power_enable(ps_output_enabled);
		...
		if (!ps_output_enabled) {
			...
			lv_label_set_text(guider_ui.scrPowerSupply_labelCurrentValue, "0.000");
			lv_label_set_text(guider_ui.scrPowerSupply_labelPowerValue, "0.000");
...
static void scrPowerSupply_sliderVoltage_event_handler (lv_event_t *e)
{
	...
	case LV_EVENT_VALUE_CHANGED:
	{
		...
		/* 发送电压设置命令到STM32 */
		stm32_power_set_voltage(ps_voltage_set);
		break;
	}
	default:
		break;
...
static void scrPowerSupply_sliderCurrent_event_handler (lv_event_t *e)
{
	...
	case LV_EVENT_VALUE_CHANGED:
	{
		...
		ps_current_set = slider_val / 1000.0f;  // Convert to current (0-2.000A)

		char current_str[16];
		snprintf(current_str, sizeof(current_str), "%.3f A", ps_current_set);
		lv_label_set_text(guider_ui.scrPowerSupply_labelCurrentSet, current_str);

		/* 发送电流设置命令到STM32 */
		stm32_power_set_current(ps_current_set);
		break;
	}
	default:
		break;
 ...
 * @param current_actual Measured current in amps (0-2A)
 */
void ps_update_current_actual(float current_actual)
{
	ps_current_actual = current_actual;
	char current_str[16];
	snprintf(current_str, sizeof(current_str), "%.3f", ps_current_actual);
...
void ps_update_power_actual(float power_actual)
{
	...
	char power_str[16];
	snprintf(power_str, sizeof(power_str), "%.3f", ps_power_actual);
	lv_label_set_text(guider_ui.scrPowerSupply_labelPowerValue, power_str);

	// Update chart with actual power (scaled by 10 for chart range 0-120, max 12W)
...
// FFT display parameters - 基于1MSPS采样率，奈奎斯特频率为500kHz
static int osc_fft_freq_range_index = 4;  // 默认500kHz（全范围）
static const float osc_fft_freq_ranges[] = {10000.0f, 50000.0f, 100000.0f, 250000.0f, 500000.0f};
static const char *osc_fft_freq_range_labels[] = {"10kHz", "50kHz", "100kHz", "250kHz", "500kHz"};
#define FFT_FREQ_RANGE_COUNT 5

static int osc_fft_amp_range_index = 2;  // 默认-60dB
static const float osc_fft_amp_ranges[] = {20.0f, 40.0f, 60.0f, 80.0f};
static const char *osc_fft_amp_range_labels[] = {"-20dB", "-40dB", "-60dB", "-80dB"};
#define FFT_AMP_RANGE_COUNT 4;

// FFT实际参数（由FFT计算时更新，供游标测量使用）
static float osc_fft_actual_sample_rate = 1000000.0f;  // 实际采样率 (Hz) - 默认1MSPS
static float osc_fft_freq_resolution = 976.56f;        // 频率分辨率 (Hz/bin) - 1M/1024
static float osc_fft_nyquist_freq = 500000.0f;         // 奈奎斯特频率 (Hz) - 500kHz
...
#define OSC_GRID_WIDTH          1024  // 统一使用1024点，与FFT一致，提升频率分辨率
#define OSC_GRID_HEIGHT         (OSC_GRID_SIZE * OSC_GRID_ROWS + 4 - 6)  // 385

// 波形显示区域实际像素尺寸（用于游标计算）
#define OSC_WAVEFORM_PIXEL_WIDTH   688   // 波形区域宽度（像素）
#define OSC_WAVEFORM_PIXEL_HEIGHT  381   // 波形区域高度（像素）
...
static void osc_waveform_update_cb(lv_timer_t *timer)
{
	...
	if (osc_use_hw_accel && osc_draw_ctx != NULL) {
		...
		if (g_osc_ctx != NULL) {
			osc_update(g_osc_ctx);
			esp_err_t ret = osc_get_display_data(g_osc_ctx, display_buffer, &display_count);
			...
			if (frame_counter % 500 == 0) {
				ESP_LOGI("OSC_UI", "Frame %lu: ret=%s, count=%lu, running=%d", 
				         ...
				         osc_get_state(g_osc_ctx) == OSC_STATE_RUNNING);
			}
		}
		
		// Prepare waveform parameters for drawing
		float volts_per_div = volt_scale_values[osc_volt_scale_index];
		
		// Calculate sample rate for FFT analysis
		float time_per_div = time_scale_values[osc_time_scale_index];
		float total_time = time_per_div * (float)OSC_GRID_COLS;
		float sample_rate = (display_count > 0 && total_time > 0) ? 
		                     (float)display_count / total_time : 0.0f;
		
		// Draw waveform or FFT
		if (osc_fft_enabled) {
			osc_fft_result_t fft_result;
			osc_draw_fft(osc_draw_ctx, display_buffer, display_count, sample_rate, &fft_result);
		} else {
			osc_draw_waveform(osc_draw_ctx, display_buffer, display_count, volts_per_div, osc_y_offset);
		}
		...
		if (g_osc_ctx != NULL) {
			...
			if (osc_fft_enabled) {
				// FFT mode - show frequency domain measurements from FFT analysis
				const osc_fft_result_t *fft_result = osc_draw_get_fft_result(osc_draw_ctx);
				
				if (fft_result != NULL && fft_result->valid) {
					// Fundamental frequency
					if (guider_ui.scrOscilloscope_labelFreqTitle != NULL) {
						if (fft_result->fundamental_freq < 0.1f) {
							snprintf(buf, sizeof(buf), "Fund: DC");
						} else if (fft_result->fundamental_freq >= 1e6f) {
							snprintf(buf, sizeof(buf), "Fund: %.2fMHz", fft_result->fundamental_freq / 1e6f);
						} else if (fft_result->fundamental_freq >= 1e3f) {
							snprintf(buf, sizeof(buf), "Fund: %.2fkHz", fft_result->fundamental_freq / 1e3f);
						} else {
							snprintf(buf, sizeof(buf), "Fund: %.1fHz", fft_result->fundamental_freq);
						}
					...
					if (guider_ui.scrOscilloscope_labelVmaxTitle != NULL) {
						if (fft_result->fundamental_freq < 0.1f) {
							snprintf(buf, sizeof(buf), "DC: %.3fV", fft_result->dc_offset);
						} else {
							snprintf(buf, sizeof(buf), "H1: %.3fV", fft_result->fundamental_amp);
						}
						lv_label_set_text(guider_ui.scrOscilloscope_labelVmaxTitle, buf);
					}
					
					// H3 (3rd harmonic amplitude)
					if (guider_ui.scrOscilloscope_labelVminTitle != NULL) {
						if (fft_result->fundamental_freq < 0.1f) {
							snprintf(buf, sizeof(buf), "H3: ---");
						} else {
							snprintf(buf, sizeof(buf), "H3: %.4fV", fft_result->h3_amp);
						}
					...
					if (guider_ui.scrOscilloscope_labelVppTitle != NULL) {
						if (fft_result->fundamental_freq < 0.1f) {
							snprintf(buf, sizeof(buf), "THD: 0.00%%");
						} else {
							snprintf(buf, sizeof(buf), "THD: %.2f%%", fft_result->thd);
						}
						lv_label_set_text(guider_ui.scrOscilloscope_labelVppTitle, buf);
					}
					
					// H2 (2nd harmonic amplitude)
					if (guider_ui.scrOscilloscope_labelVrmsTitle != NULL) {
						if (fft_result->fundamental_freq < 0.1f) {
							snprintf(buf, sizeof(buf), "H2: ---");
						} else {
							snprintf(buf, sizeof(buf), "H2: %.4fV", fft_result->h2_amp);
						}
						lv_label_set_text(guider_ui.scrOscilloscope_labelVrmsTitle, buf);
					}
				} else {
					// No valid FFT results - show waiting message
					if (guider_ui.scrOscilloscope_labelFreqTitle != NULL) {
					...
					if (guider_ui.scrOscilloscope_labelVrmsTitle != NULL) {
						lv_label_set_text(guider_ui.scrOscilloscope_labelVrmsTitle, "H2: ---");
					}
				}
			} else {
				// Time domain mode - show normal measurements
				osc_measurement_t meas;
				if (osc_get_measurements(g_osc_ctx, &meas) == ESP_OK) {
					// Frequency
					if (guider_ui.scrOscilloscope_labelFreqTitle != NULL) {
						if (meas.freq >= 1e6f) {
							snprintf(buf, sizeof(buf), "Freq: %.2fMHz", meas.freq / 1e6f);
						} else if (meas.freq >= 1e3f) {
							snprintf(buf, sizeof(buf), "Freq: %.2fkHz", meas.freq / 1e3f);
						} else {
							snprintf(buf, sizeof(buf), "Freq: %.1fHz", meas.freq);
						}
						lv_label_set_text(guider_ui.scrOscilloscope_labelFreqTitle, buf);
					}
					...
					// Vmax
					if (guider_ui.scrOscilloscope_labelVmaxTitle != NULL) {
						snprintf(buf, sizeof(buf), "Vmax: %.2fV", meas.vmax);
						lv_label_set_text(guider_ui.scrOscilloscope_labelVmaxTitle, buf);
					}
					
					// Vmin
					if (guider_ui.scrOscilloscope_labelVminTitle != NULL) {
						snprintf(buf, sizeof(buf), "Vmin: %.2fV", meas.vmin);
						lv_label_set_text(guider_ui.scrOscilloscope_labelVminTitle, buf);
					}
					
					// Vpp
					if (guider_ui.scrOscilloscope_labelVppTitle != NULL) {
						snprintf(buf, sizeof(buf), "Vp-p: %.2fV", meas.vpp);
						lv_label_set_text(guider_ui.scrOscilloscope_labelVppTitle, buf);
					}
					
					// Vrms
					if (guider_ui.scrOscilloscope_labelVrmsTitle != NULL) {
						snprintf(buf, sizeof(buf), "Vrms: %.2fV", meas.vrms);
						lv_label_set_text(guider_ui.scrOscilloscope_labelVrmsTitle, buf);
					}
				} else {
	...
	// 动态获取图表的实际点数，防止越界访问
	uint16_t chart_point_count = lv_chart_get_point_count(guider_ui.scrOscilloscope_chartWaveform);
	
	// Chart display parameters - 43x43 pixel grid, 16x9 divisions
	// 使用实际的图表点数，而不是硬编码的值
	const int num_points = (int)chart_point_count;
	...
	if (!osc_running && osc_frozen_data_valid) {
		...
		// 注意：osc_frozen_voltage_data 数组大小是 OSC_GRID_WIDTH (1024)
		const int frozen_data_size = OSC_GRID_WIDTH;
		...
		for(int i = 0; i < num_points; i++) {
			...
			// 同时确保不会越界访问 osc_frozen_voltage_data 数组
			if (source_index < 0 || source_index >= frozen_data_size) {
				// 超出范围，显示为零（或者可以显示为空白）
				voltage = 0.0f;
			} else {
				// Linear interpolation for smooth scaling
				float frac = source_pos - (float)source_index;
				if (source_index + 1 < frozen_data_size && frac > 0.0f) {
	...
	const int fft_display_points = 1024;  // Number of bars in FFT spectrum

	if (osc_fft_enabled) {
		// FFT mode - show frequency spectrum using REAL ADC data
		
		// 重要：FFT模式下停止时，不更新数据，保持最后的FFT显示
		// 这样可以避免在停止时访问无效数据导致崩溃
		if (!osc_running) {
			// FFT模式下停止：保持当前显示，不做任何更新
			// 只刷新图表以保持显示
			lv_chart_refresh(guider_ui.scrOscilloscope_chartWaveform);
			return;
		}
		...
		// Get actual sampling rate from oscilloscope core (default 1MSPS)
		float sampling_rate = 1000000.0f;  // Default 1MSPS for 500kHz FFT
		if (g_osc_ctx != NULL) {
			uint32_t actual_rate = osc_get_sample_rate_hz(g_osc_ctx);
			if (actual_rate > 0) {
		...
		float freq_resolution = sampling_rate / 1024.0f;  // FFT bin resolution (Hz per bin)
		float nyquist_freq = sampling_rate / 2.0f;  // Maximum displayable frequency (Nyquist)
		float signal_frequency = 0.0f;  // Will be calculated from FFT peak
		
		// 更新全局FFT参数（供游标测量使用）
		osc_fft_actual_sample_rate = sampling_rate;
		osc_fft_freq_resolution = freq_resolution;
		osc_fft_nyquist_freq = nyquist_freq;
		...
		if (g_osc_ctx != NULL) {
			...
			if (ret == ESP_OK && display_count > 0) {
				...
				const int fft_size = 1024;  // Use 1024 points for FFT (power of 2)
				
				// ESP-DSP requires aligned buffers
				__attribute__((aligned(16))) static float fft_input_real[1024];
				__attribute__((aligned(16))) static float fft_input_imag[1024];
				__attribute__((aligned(16))) static float fft_output[1024];
				static float fft_magnitude[512];  // Only need N/2 for real signal
				...
				// 首先检查DC分量（bin 0）
				float dc_magnitude = fft_magnitude[0];
				
				// 找AC分量的最大值（从bin 2开始，跳过bin 1的低频噪声）
				float max_ac_magnitude = 0.0f;
				int peak_bin = 0;
				for (int i = 2; i < fft_size / 2; i++) {
					if (fft_magnitude[i] > max_ac_magnitude) {
						max_ac_magnitude = fft_magnitude[i];
						peak_bin = i;
					}
				}
				
				// 也检查bin 1
				if (fft_magnitude[1] > max_ac_magnitude) {
					max_ac_magnitude = fft_magnitude[1];
					peak_bin = 1;
				}
				...
				// 1. DC分量是AC峰值的5倍以上
				// 2. 或者AC峰值太小（噪声级别）
				// 3. 或者峰值在bin 1（接近DC的低频）且DC分量较大
				float max_magnitude;
				bool is_dc_signal = false;
				
				if (dc_magnitude > max_ac_magnitude * 5.0f) {
					is_dc_signal = true;
				} else if (max_ac_magnitude < 0.005f) {
					is_dc_signal = true;
				} else if (peak_bin == 1 && dc_magnitude > max_ac_magnitude * 2.0f) {
					is_dc_signal = true;
				}
				...
				if (is_dc_signal) {
					...
					max_magnitude = (dc_magnitude > 0.001f) ? dc_magnitude : 0.001f;
					peak_bin = 0;
				} else {
					// AC信号
					signal_frequency = peak_bin * freq_resolution;
					max_magnitude = max_ac_magnitude;
				}
		...
		// FFT size - 1024点FFT
		if (guider_ui.scrOscilloscope_labelVrmsTitle != NULL) {
			snprintf(buf, sizeof(buf), "FFT: 1024pt");
			lv_label_set_text(guider_ui.scrOscilloscope_labelVrmsTitle, buf);
		}
		...
		if (display_count > 0) {
			...
			float freq_hz = 0.0f;
			osc_measurement_t meas;
			if (g_osc_ctx != NULL && osc_get_measurements(g_osc_ctx, &meas) == ESP_OK) {
				freq_hz = meas.freq;
			}
...
static void scrOscilloscope_event_handler (lv_event_t *e)
{
	...
	case LV_EVENT_SCREEN_LOADED:
	{
		// Initialize oscilloscope (real ADC sampling)
		esp_err_t ret = osc_global_init();
		if (ret != ESP_OK) {
			ESP_LOGE("OSC", "Failed to initialize oscilloscope: %s", esp_err_to_name(ret));
		}
		
		/* 切换STM32到示波器模式，开始上报波形数据 */
		stm32_set_mode(STM32_MODE_OSCILLOSCOPE);
		ESP_LOGI("OSC_UI", "Oscilloscope screen loaded, mode switched to OSCILLOSCOPE");
		...
		// 同步初始时基和电压档位到核心层
		if (g_osc_ctx != NULL) {
			osc_set_time_per_div(g_osc_ctx, time_scale_values[osc_time_scale_index]);
			osc_set_volts_per_div(g_osc_ctx, volt_scale_values[osc_volt_scale_index]);
		}
	...
	case LV_EVENT_SCREEN_UNLOADED:
	{
		/* 页面卸载时切换回空闲模式 */
		stm32_set_mode(STM32_MODE_IDLE);
		ESP_LOGI("OSC_UI", "Oscilloscope screen unloaded, mode switched to IDLE");
		...
		// Deinitialize oscilloscope (stop ADC sampling)
		osc_global_deinit();
...
static void scrOscilloscope_btnStartStop_event_handler (lv_event_t *e)
{
	...
	case LV_EVENT_CLICKED:
	{
		...
		if (osc_running) {
			...
			// 恢复定时器（如果之前被暂停）
			if (osc_waveform_timer != NULL) {
				lv_timer_resume(osc_waveform_timer);
			}
		} else {
			// 停止运行 - 停止ADC采样，冻结波形
			...
			// 停止真实ADC采样
			osc_stop(g_osc_ctx);
			// 在FFT模式下，暂停定时器以防止访问无效数据
			if (osc_fft_enabled && osc_waveform_timer != NULL) {
				lv_timer_pause(osc_waveform_timer);
			}
			// 冻结数据已在波形更新回调中保存
		}
		break;
...
static void scrOscilloscope_btnFFT_event_handler (lv_event_t *e)
{
	...
	case LV_EVENT_CLICKED:
	{
		...
		if (osc_fft_enabled) {
			...
			if (!guider_ui.scrOscilloscope_chartWaveform || !lv_obj_is_valid(guider_ui.scrOscilloscope_chartWaveform)) return;
			...
			// 先设置点数，再切换类型（顺序很重要）
			lv_chart_set_point_count(guider_ui.scrOscilloscope_chartWaveform, 1024);
			...
			// 设置柱状图样式
			lv_obj_set_style_pad_column(guider_ui.scrOscilloscope_chartWaveform, 0, LV_PART_ITEMS);  // 柱子间距0px，更密集
			...
			// Update title labels for FFT mode
			if (guider_ui.scrOscilloscope_labelTimeScaleTitle && lv_obj_is_valid(guider_ui.scrOscilloscope_labelTimeScaleTitle))
				lv_label_set_text(guider_ui.scrOscilloscope_labelTimeScaleTitle, "Span");
			if (guider_ui.scrOscilloscope_labelVoltScaleTitle && lv_obj_is_valid(guider_ui.scrOscilloscope_labelVoltScaleTitle))
				lv_label_set_text(guider_ui.scrOscilloscope_labelVoltScaleTitle, "dB");
			...
			// 初始化所有数据点为中心值，防止显示垃圾数据
			if (ser != NULL) {
				for (int i = 0; i < OSC_GRID_WIDTH; i++) {
					ser->y_points[i] = 500;  // 中心值
				}
			}
			...
			// Restore time domain title labels
			if (guider_ui.scrOscilloscope_labelTimeScaleTitle && lv_obj_is_valid(guider_ui.scrOscilloscope_labelTimeScaleTitle))
				lv_label_set_text(guider_ui.scrOscilloscope_labelTimeScaleTitle, "Time");
			if (guider_ui.scrOscilloscope_labelVoltScaleTitle && lv_obj_is_valid(guider_ui.scrOscilloscope_labelVoltScaleTitle))
				lv_label_set_text(guider_ui.scrOscilloscope_labelVoltScaleTitle, "Volt");
			
			// Restore time domain labels
			// Update time scale label back to time
			if (guider_ui.scrOscilloscope_labelTimeScaleValue && lv_obj_is_valid(guider_ui.scrOscilloscope_labelTimeScaleValue)) {
				const char *time_str = osc_get_time_scale_str((osc_time_scale_t)osc_time_scale_index);
				lv_label_set_text(guider_ui.scrOscilloscope_labelTimeScaleValue, time_str);
			}
			
			// Update voltage scale label back to voltage
			if (guider_ui.scrOscilloscope_labelVoltScaleValue && lv_obj_is_valid(guider_ui.scrOscilloscope_labelVoltScaleValue)) {
				const char *volt_str = osc_get_volt_scale_str((osc_volt_scale_t)osc_volt_scale_index);
		...
		if (osc_export_enabled) {
			...
			if (ser != NULL) {
				...
				if (osc_fft_enabled) {
					...
					fft_data.vrms = 1024.0f;  // FFT size
...
static void scrOscilloscope_contTimeScale_event_handler (lv_event_t *e)
{
	...
	case LV_EVENT_CLICKED:
	{
		...
		if (osc_fft_enabled) {
			...
			osc_fft_freq_range_index = (osc_fft_freq_range_index + 1) % FFT_FREQ_RANGE_COUNT;
			lv_label_set_text(guider_ui.scrOscilloscope_labelTimeScaleValue, osc_fft_freq_range_labels[osc_fft_freq_range_index]);
			return;
		}
...
static void scrOscilloscope_contVoltScale_event_handler (lv_event_t *e)
{
	...
	case LV_EVENT_CLICKED:
	{
		...
		if (osc_fft_enabled) {
			...
			osc_fft_amp_range_index = (osc_fft_amp_range_index + 1) % FFT_AMP_RANGE_COUNT;
			lv_label_set_text(guider_ui.scrOscilloscope_labelVoltScaleValue, osc_fft_amp_range_labels[osc_fft_amp_range_index]);
			return;
		}
		
		// Cycle through voltage scales: 10mV, 20mV, 50mV, 100mV, 200mV, 500mV, 1V, 2V, 5V, 10V
		osc_volt_scale_index = (osc_volt_scale_index + 1) % VOLT_SCALE_COUNT;
		lv_label_set_text(guider_ui.scrOscilloscope_labelVoltScaleValue, volt_scale_labels[osc_volt_scale_index]);
...
static void scrOscilloscope_contTrigger_event_handler (lv_event_t *e)
{
	...
	case LV_EVENT_CLICKED:
	{
		...
		if (osc_cursor_mode == OSC_CURSOR_OFF) {
			...
			osc_cursor_position = OSC_WAVEFORM_PIXEL_WIDTH / 2;  // 中心位置
			line_points_h[0].x = osc_cursor_position;
			line_points_h[0].y = 0;
			line_points_h[1].x = osc_cursor_position;
			line_points_h[1].y = OSC_WAVEFORM_PIXEL_HEIGHT;
			...
			osc_cursor_position = OSC_WAVEFORM_PIXEL_HEIGHT / 2;  // 中心位置
			line_points_v[0].x = 0;
			line_points_v[0].y = osc_cursor_position;
			line_points_v[1].x = OSC_WAVEFORM_PIXEL_WIDTH;
...
static void scrOscilloscope_btnAuto_event_handler (lv_event_t *e)
{
	...
	case LV_EVENT_CLICKED:
	{
		...
		if (ser != NULL) {
			...
			const int num_points = OSC_GRID_WIDTH;  // 1024 data points
			const float volt_scale_values[] = {0.01, 0.02, 0.05, 0.1, 0.2, 0.5, 1.0, 2.0, 5.0, 12.0};
	...
	case LV_EVENT_PRESSING:
	{
		...
		if (osc_cursor_mode != OSC_CURSOR_OFF && osc_cursor_line != NULL) {
			...
			if (osc_cursor_mode == OSC_CURSOR_HORIZONTAL) {
				// 横轴游标 - 更新 X 位置
				if (rel_x < 0) rel_x = 0;
				if (rel_x > OSC_WAVEFORM_PIXEL_WIDTH) rel_x = OSC_WAVEFORM_PIXEL_WIDTH;
				...
				h_line_points[1].y = OSC_WAVEFORM_PIXEL_HEIGHT;
				lv_line_set_points(osc_cursor_line, h_line_points, 2);
				
				// 更新标签位置和值
				int label_x = osc_cursor_position + 8;
				// 防止标签超出右边界
				if (label_x > OSC_WAVEFORM_PIXEL_WIDTH - 80) label_x = osc_cursor_position - 70;
				...
				if (osc_fft_enabled) {
					// FFT 模式 - 显示频率
					// 与FFT绘制使用完全相同的映射：
					// FFT绘制: for i in 0..1023: freq = i * max_freq / 1024
					// 屏幕: 688像素显示1024个点
					
					// 直接计算：像素位置对应的频率
					float max_freq = osc_fft_freq_ranges[osc_fft_freq_range_index];
					float display_freq = (float)osc_cursor_position * max_freq / (float)OSC_WAVEFORM_PIXEL_WIDTH;
					
					// 格式化频率显示
					if (display_freq >= 1e6f) {
						snprintf(buf, sizeof(buf), "%.2fMHz", display_freq / 1e6f);
					} else if (display_freq >= 1e3f) {
						snprintf(buf, sizeof(buf), "%.2fkHz", display_freq / 1e3f);
					} else {
						snprintf(buf, sizeof(buf), "%.1fHz", display_freq);
					}
				} else {
					// 时域模式 - 显示时间
					float time_per_div = time_scale_values[osc_time_scale_index];
					float total_time = time_per_div * OSC_GRID_COLS;
					float time = (float)osc_cursor_position * total_time / (float)OSC_WAVEFORM_PIXEL_WIDTH;
					if (time >= 1.0f) {
				...
				// 纵轴游标 - 更新 Y 位置
				if (rel_y < 0) rel_y = 0;
				if (rel_y > OSC_WAVEFORM_PIXEL_HEIGHT) rel_y = OSC_WAVEFORM_PIXEL_HEIGHT;
				...
				v_line_points[1].x = OSC_WAVEFORM_PIXEL_WIDTH;
				...
				if (osc_fft_enabled) {
					// FFT 模式 - 显示幅值（dB）
					// Chart Y轴：0=底部(最小dB), 1000=顶部(0dB)
					// 屏幕Y轴：0=顶部, HEIGHT=底部
					// 映射：屏幕Y -> chart Y -> dB
					float chart_y = (float)(OSC_WAVEFORM_PIXEL_HEIGHT - osc_cursor_position) * 1000.0f / (float)OSC_WAVEFORM_PIXEL_HEIGHT;
					float amp_range = osc_fft_amp_ranges[osc_fft_amp_range_index];
					// chart_y=1000 -> 0dB, chart_y=0 -> -amp_range dB
					float db = (chart_y / 1000.0f) * amp_range - amp_range;
					snprintf(buf, sizeof(buf), "%.1fdB", db);
				} else {
					// 时域模式 - 显示电压
					float volts_per_div = volt_scale_values[osc_volt_scale_index];
					// 屏幕坐标：Y=0 在顶部，Y=HEIGHT 在底部，中心=HEIGHT/2
					// 中心对应 0V，每格对应 volts_per_div
					float center_y = (float)OSC_WAVEFORM_PIXEL_HEIGHT / 2.0f;
					float pixels_per_div = (float)OSC_WAVEFORM_PIXEL_HEIGHT / (float)OSC_GRID_ROWS;
					float voltage = (center_y - (float)osc_cursor_position) / pixels_per_div * volts_per_div;
