...
void setup_scr_scrOscilloscope(lv_ui *ui)
{
    ...
    lv_chart_set_point_count(ui->scrOscilloscope_chartWaveform, 512);  // 统一使用512点，与FFT一致
    ...
    // 使用512点（与lv_chart_set_point_count一致）
    for(int i = 0; i < 512; i++) ser1->y_points[i] = 500;
