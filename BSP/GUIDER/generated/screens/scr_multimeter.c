...
static void dmm_current_btn_event_cb(lv_event_t * e)
{
    ...
    // 发送模式切换命令到STM32
    stm32_meter_set_mode(METER_MODE_CURRENT);
...
static void dmm_resistance_btn_event_cb(lv_event_t * e)
{
    ...
    // 发送模式切换命令到STM32
    stm32_meter_set_mode(METER_MODE_RESISTANCE);
...
static void dmm_range_btn_event_cb(lv_event_t * e)
{
    ...
    if (dmm_state.mode == DMM_MODE_RESISTANCE) {
        ...
        // 发送量程切换命令到STM32
        stm32_meter_set_range(dmm_state.res_range);
...
void setup_scr_scrPrintMenu(lv_ui *ui)
{
	...
	// 注意：模拟数据定时器已禁用，使用STM32真实数据
	// 真实数据通过 custom.c 中的 init_digital_multimeter() 注册的回调接收
