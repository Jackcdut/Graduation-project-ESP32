...
static void current_btn_event_cb(lv_event_t * e)
{
	...
	if (new_val < 0) new_val = 0;
	if (new_val > 2000) new_val = 2000;  // 0-2A range
...
void setup_scr_scrPowerSupply(lv_ui *ui)
{
	...
	lv_label_set_text(ui->scrPowerSupply_labelCurrentSet, "1.000 A");
	...
	lv_textarea_set_text(current_input_ta, "1.000");
	...
	lv_slider_set_range(ui->scrPowerSupply_sliderCurrent, 0, 2000);  // 0-2.000A
	lv_slider_set_value(ui->scrPowerSupply_sliderCurrent, 1000, LV_ANIM_OFF);
