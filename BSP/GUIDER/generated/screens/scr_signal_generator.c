...
void setup_scr_scrCopy(lv_ui *ui)
{
	...
	/* Apply button - sends signal generator settings to STM32 */
	ui->scrCopy_btnApply = lv_btn_create(status_section);
	lv_obj_align(ui->scrCopy_btnApply, LV_ALIGN_CENTER, 0, 5);
	lv_obj_set_size(ui->scrCopy_btnApply, 150, 36);
	lv_obj_set_style_bg_opa(ui->scrCopy_btnApply, 0, LV_PART_MAIN|LV_STATE_DEFAULT);
	lv_obj_set_style_border_width(ui->scrCopy_btnApply, 0, LV_PART_MAIN|LV_STATE_DEFAULT);
	lv_obj_set_style_shadow_width(ui->scrCopy_btnApply, 0, LV_PART_MAIN|LV_STATE_DEFAULT);
	lv_obj_set_style_bg_color(ui->scrCopy_btnApply, lv_color_hex(0xE8F5E9), LV_PART_MAIN|LV_STATE_PRESSED);
	lv_obj_set_style_bg_opa(ui->scrCopy_btnApply, 255, LV_PART_MAIN|LV_STATE_PRESSED);
	lv_obj_set_style_radius(ui->scrCopy_btnApply, 8, LV_PART_MAIN);
	
	lv_obj_t * apply_label = lv_label_create(ui->scrCopy_btnApply);
	lv_label_set_text(apply_label, LV_SYMBOL_OK " APPLY");
	lv_obj_align(apply_label, LV_ALIGN_CENTER, 0, 0);
	lv_obj_set_style_text_font(apply_label, &lv_font_montserratMedium_20, LV_PART_MAIN);
	lv_obj_set_style_text_color(apply_label, lv_color_hex(0x2E7D32), LV_PART_MAIN);
