...
static esp_lcd_panel_handle_t g_panel_handle = NULL;  // Global panel handle for screenshot
...
esp_err_t bsp_audio_init(const i2s_std_config_t *i2s_config)
{
    ...
    /* Setup I2S peripheral with maximum DMA buffer for video playback
     * At 16kHz stereo 16bit = 64KB/s, we need enough buffer to survive video frame processing
     * 16 descriptors × 1008 frames × 4 bytes = 64KB = ~1 second buffer at 16kHz */
    i2s_chan_config_t chan_cfg = I2S_CHANNEL_DEFAULT_CONFIG(CONFIG_BSP_I2S_NUM, I2S_ROLE_MASTER);
    chan_cfg.auto_clear = true; // Auto clear the legacy data in the DMA buffer
    chan_cfg.dma_desc_num = 16;   // Maximum DMA descriptors
    chan_cfg.dma_frame_num = 1008; // Max allowed frames per descriptor
...
esp_err_t bsp_display_new_with_handles(const bsp_display_config_t *config, bsp_lcd_handles_t *ret_handles)
{
    ...
    /* DPI config - 480x800 portrait panel */
...
esp_err_t bsp_touch_new(const bsp_touch_config_t *config, esp_lcd_touch_handle_t *ret_touch)
{
    ...
    const esp_lcd_touch_config_t tp_cfg = {
        .x_max = BSP_LCD_H_RES,  /* 480 - physical touch resolution */
        .y_max = BSP_LCD_V_RES,  /* 800 - physical touch resolution */
        ...
        .flags = {
            .swap_xy = 0,  /* No swap - LVGL software rotation will handle it */
#if CONFIG_BSP_LCD_TYPE_1024_600
            .mirror_x = 0,
            .mirror_y = 0,
#else
...
static lv_display_t *bsp_display_lcd_init(const bsp_display_cfg_t *cfg)
{
    ...
    /* Save panel handle globally for screenshot access */
    g_panel_handle = lcd_panels.panel;
    ...
    const lvgl_port_display_cfg_t disp_cfg = {
        ...
        /* Register physical panel resolution: 480x800 (portrait) */
        .hres = BSP_LCD_H_RES,  /* 480 */
        .vres = BSP_LCD_V_RES,  /* 800 */
        .monochrome = false,
        /* No rotation here - will use bsp_display_rotate() in main.c */
        .rotation = {
            .swap_xy = false,
            .mirror_x = false,
            .mirror_y = false,
...
#endif
#if CONFIG_BSP_DISPLAY_LVGL_AVOID_TEAR && !CONFIG_BSP_DISPLAY_LVGL_USE_VSYNC
            .sw_rotate = false,                /* Full avoid tearing mode without VSync is not supported for SW rotation */
#else
            .sw_rotate = cfg->flags.sw_rotate, /* SW/PPA rotation is supported with VSync-only mode */
#endif
