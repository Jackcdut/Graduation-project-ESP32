...
#include "freertos/idf_additions.h"  /* For xTaskCreatePinnedToCoreWithCaps */
...
void *hosted_thread_create(char *tname, uint32_t tprio, uint32_t tstack_size, void (*start_routine)(void const *), void *sr_arg)
{
	...
	*thread_handle = osThreadCreate(osThread(Ctrl_port_tsk), arg);
#endif
	/* Use xTaskCreatePinnedToCoreWithCaps to allocate stack in PSRAM */
	/* This prevents internal RAM exhaustion during ESP-Hosted initialization */
	task_created = xTaskCreatePinnedToCoreWithCaps(
		...
		MALLOC_CAP_SPIRAM  /* Allocate stack in PSRAM */
	);
	if (!(*thread_handle)) {
